...
interface InfoTileProps { label: string; value: string | number; color?: string; }
...
const ProductDetailPage: React.FC<Props> = ({ id }) => {
        ...
        <ul className="flex flex-col gap-2">
          {product.features.map((feat, i) => (
            <li key={i} className="flex items-center gap-2.5 text-sm text-slate-200">
              <span className="w-5 h-5 rounded-full bg-accent/15 text-accent2 flex items-center justify-center text-[11px] font-bold shrink-0">✓</span>
              {feat}
            </li>
          ))}
        </ul>
